          ...
          <div class="ml-3 relative">
            <div>
              <button type="button" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                ...
                <img v-if="userAvatar" class="h-8 w-8 rounded-full" :src="userAvatar" alt="">
...
const store = computed(() => userStore.getState())
const userAvatar = computed(() => {
  if (userStore.getState()?.user?.email){
    return `https://www.gravatar.com/avatar/${MD5(userStore.getState().user.email)}`
  }
